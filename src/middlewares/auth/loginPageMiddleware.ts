...
const loginPageMiddleware = async (
        ...
        if (clientId == null || redirect_uri == null) {
            return res.send('Bad Request: RedirectUri or clientId not provided');
        }
        
        logger.info('attempting to authenticate user');
        const authenticationResult = await authenticationService.authenticateUser(username, password);
        ...
        if (authenticationResult) {
            ...
            await authenticationEventService.createAuthenticationEvent(username, authorizationCode);
